...
<script lang="ts">
  ...
  $: styleForMap = (i: number) => {
    ...
    return `width:${newLineAtBeat}ch; top: calc(-${
      activeBanks!.length - 1
    }ch/2 + ${i}ch); line-height: ${activeBanks!.length}ch;`;
  };
</script>
...
<div class="header" bind:this={header}>
  ...
  <HuesButton
    ...
    on:click={() => dispatch("rewind")}>{@html HuesIcon.REWIND}</HuesButton
  >
  ...
  <HuesButton
    icon
    disabled={!section?.sound}
    title="Copy formatted beatmap"
    on:click={() => copyPretty()}>{@html HuesIcon.COPY}</HuesButton
  >
  ...
  <HuesButton
    ...
    disabled={!section?.sound || locked || section.mapLen <= 1}
  >
    Halve
  </HuesButton>
  <HuesButton
    on:click={() => dispatch("double")}
    disabled={!section?.sound || locked}
  >
    Double
  </HuesButton>
  <input
  ...
  <HuesButton on:click={removeSong} disabled={!section?.sound}
    >Remove</HuesButton
  >
</div>
