<script>
    ...
    let drawWave = () => {
        ...
        if(buildWave && bLen && minTime < 0) {
            ...
            let waveOffset = Math.floor((1 - drawTime / -bLen) * (buildWave.width-1));
            try {
        ...
        if(loopWave && loopLen && maxTime > 0) {
            while(drawOffset < width) {
                ...
                let waveOffset = Math.floor((drawTime / loopLen) * (loopWave.width-1));
    ...
    $: if(soundManager) {
        buildWave = renderWave(soundManager.build.buffer, soundManager.build.length);
